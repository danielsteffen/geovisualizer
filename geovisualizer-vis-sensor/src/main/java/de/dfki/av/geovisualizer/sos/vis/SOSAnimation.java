...
/**
 * Animation to visualize SOS live data.
 *
 * @author Tobias Zimmermann <tobias.zimmermann at dfki.de>
 */
public class SOSAnimation implements Renderable {

    /**
     * The logger.
     */
    private static final Logger log = LoggerFactory.getLogger(SOSAnimation.class);
     ...
     * Constructor.
     */
    public SOSAnimation(RenderableLayer layer) {
    ...
    private void initialize(DrawContext dc) {
        ...
        // setup vertices
        FloatBuffer vertices = Buffers.newDirectFloatBuffer(bufferSize * 12);
        for (int i = 0; i < latlonList.size(); i++) {
        ...
        FloatBuffer colors = Buffers.newDirectFloatBuffer(bufferSize * 4 * 3);
        // Side color
        for (int i = 0; i < latlonList.size(); i++) {
    ...
    private void updateVBO(DrawContext dc) {
        ...
        if (byteBuffer != null) {
            ...
            for (int i = 0; i < latlonList.size(); i++) {
                LatLon latLon = latlonList.get(i);
                double elevation = terrainElevation[i] + elevationsList.get(i);
                List<Vec4> vectors;
                Vec4 top = globe.computePointFromPosition(latLon, elevation);
                vectors = new ArrayList<>();
                vectors.add(top);
    ...
    private void updateAnnotations() {
        ...
        if (gaNoStress != null && gaStress != null && gaTime != null && !latlonList.isEmpty()) {
            ...
            if (stressList.get(index - 1) > 0) {
                ...
                for (int i = 0; i < 25; i++) {
                    if (index - i > 0) {
                        if (index - 1 > -1 && stressList.get(index - 1 - i) > 0) {
                            fade = true;
