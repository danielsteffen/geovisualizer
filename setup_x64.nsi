...
!define NATIVEDIR "natives\x64" 
 
; Definitions for Java 7.0 local
!define JRE_VERSION "7.0"
!define JRE_URL "$INSTDIR\jre-7u9-windows-x64.exe"
...
;create start-menu items
  CreateDirectory "$PROGRAMFILES64\${PROJECT_NAME}"
  CreateShortCut "$PROGRAMFILES64\${PROJECT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$PROGRAMFILES64\${PROJECT_NAME}\${PROJECT_NAME}.lnk" "$INSTDIR\${MUI_FILE}" "" "$INSTDIR\${MUI_FILE}" 0
