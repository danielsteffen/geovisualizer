 ...
 */
public class WaterlevelTest extends ApplicationTemplate {
...
//                    createPrecipitationSurface(HUE_BLUE, HUE_RED, analyticSurfaceLayer);
    ...
    protected static void createWaterLevel(double minHue, double maxHue, int width, int height,
            RenderableLayer outLayer) {
        double minValue = 0;
        double maxValue = 1;
...
//        surface.setSector(Sector.fromDegrees(50, 60, 7, 17));
        surface.setAltitude(230);
        surface.setDimensions(width, height);
        surface.setClientLayer(outLayer);
        outLayer.addRenderable(surface);
...
//        mixValuesOverTime(2000L, firstBuffer, secondBuffer, minValue, maxValue, minHue, maxHue, surface);
        ...
        final ArrayList<AnalyticSurface.GridPointAttributes> attributesList = new ArrayList<AnalyticSurface.GridPointAttributes>();

        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                double value = Math.random() + minValue;
...
//                double opacity = WWMath.computeInterpolationFactor(value, minValue, minValue + (maxValue - minValue) * 0.1);
                Color rgbaColor = new Color(color.getRed(), color.getGreen(), color.getBlue(), 255);
                attributesList.add(
                        AnalyticSurface.createGridPointAttributes(value, rgbaColor));
            }
        }
