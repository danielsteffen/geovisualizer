...
public class DEMLoader extends AbstractLoader {
    ...
    @Override
    public Scene load(final Reader reader) throws
...
//        logger.debug("triangleIndex: " + (currentTriangle * 3 + 1) + " gridpointindex: (" + (currentColumn + 1) + "," + (currentRow + 1) + ")");
                triangleCoordinates[(currentTriangle * 3) + 2] =
                        arcGrid.getGridPoint(currentColumn + 1, currentRow + 1);
//        logger.debug("triangleIndex: " + (currentTriangle * 3 + 2) + " gridpointindex: (" + currentColumn +"," + (currentRow + 1) + ")");
                triangleCoordinates[(currentTriangle * 3) + 1] = arcGrid.getGridPoint(currentColumn, currentRow + 1);
                currentTriangle++;
//        logger.debug("triangleIndex: " + (currentTriangle * 3 + 2) + " gridpointindex: (" + (currentColumn + 1) + "," + (currentRow + 1) + ")");
                triangleCoordinates[(currentTriangle * 3)] = arcGrid.getGridPoint(currentColumn + 1, currentRow + 1);
...
//        logger.debug("triangleIndex: " + (currentTriangle * 3) + " gridpointindex: (" + currentColumn + "," + (currentRow) + ")");
                triangleCoordinates[(currentTriangle * 3) + 2] = arcGrid.getGridPoint(currentColumn, currentRow);
                currentTriangle++;
            }
        }
        if (logger.isDebugEnabled()) {
...
//    pa.setCullFace(PolygonAttributes.CULL_NONE);
        ...
        material.setDiffuseColor(new Color3f(0.65f, 0.65f, 0.65f));
