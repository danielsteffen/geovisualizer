...
import static com.dfki.av.sudplan.io.RawArcGrid.*;
...
//  private float zExaggeration =10f;
  private float scaleFactor = 0.01f;
  private float zExaggeration = 10f;
...
//      arcGrid.setNumberOfColumns(100);
      ...
      while (sr.ready() & rowCount < arcGrid.getNumberOfRows()) {
        ...
        for (int currentColumn = 0; currentColumn < arcGrid.getNumberOfColumns(); currentColumn++) {
          ...
          pointList[coordinateCount] = new Point3f((x) * (new Float(arcGrid.getCellsize()) * scaleFactor), (y)
                  * (new Float(arcGrid.getCellsize()) * scaleFactor), z * scaleFactor * zExaggeration);
          x++;
