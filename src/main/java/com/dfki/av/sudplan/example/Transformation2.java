 ...
 * @since 1.6
 */
//Example how to scale something on the same position
public class Transformation2 extends JPanel {

    private final static Logger logger = LoggerFactory.getLogger(Transformation2.class);
...
//        TransformGroup objRotate = new TransformGroup(transformObject());
        ...
        final Vector3d translate1 = new Vector3d(-2, 0, 0);
        final Transform3D translation1 = new Transform3D();
        translation1.setTranslation(translate1);
        final Vector3d translate2 = new Vector3d(2, 0, 0);
        final Transform3D translation2 = new Transform3D();
        translation2.setTranslation(translate2);
        ...
        final Transform3D scaling = new Transform3D();
...
//                new RotationInterpolator(rotationAlpha, objRotate);
        final Shape3D cone = createCone(0.5f, coneLength);
        printButton.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                printNodePosition(cone);
                printNodePosition(objRotate);
            }
        });
...
//        targetTrans.mul(rotation);        
//        targetTrans.mul(translation2);
//        targetTrans.mul(scaling);
        targetTrans.mul(translation1);
//        objRotate.getTransform(targetTrans);
//        targetTrans.set(translate);
        objRotate.setTransform(targetTrans);
        printNodePosition(cone);
...
//        simpleU.getViewingPlatform().setViewPlatformBehavior(behavior);
        ...
        printNodePosition(cone);
        printNodePosition(objRotate);
        final AdvancedBoundingBox bounds = new AdvancedBoundingBox(cone.getBounds());
        if (logger.isDebugEnabled()) {
            logger.debug("center: "+bounds.getCenter());
        }
    }
...
//    } // end of CreateSceneGraph method of HelloJava3Da
    ...
    private void printNodePosition(final Node node) {
